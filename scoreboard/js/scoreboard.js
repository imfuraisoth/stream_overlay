...
function scoreboard(scObj){
	...
	if(startup == true){
		...
		getData(scObj); //runs function that sets data polled from json into html objects
		startup = false; //flags that the scoreboard/getData functions have run their first pass
	} else{
		getData(scObj); //if startup is not set to true, only the getData function is run each time scoreboard function runs
	}
}
...
window.setInterval(pollJSON, 2000);