...
function init(){
    ...
    function displayPlacement(placementData, player) {
		...
		if (placementData["placement"] != 1) {
			element.style.color = "#FFD2D2";
		}
	...
	function removePlacement(player) {
        // Remove ALL matching elements -- getElementById returns only the
        // first, so two displayPlacement calls landing within the ~1s async
        // removal window (round cycle + OBS scene switch coalescing polls)
        // could orphan a ghost placement that stayed one update behind.
        document.querySelectorAll('[id="placementElementText' + player + '"]').forEach(function (el) {
            removeElement(el, 700);
        });
        document.querySelectorAll('[id="placementImage' + player + '"]').forEach(function (el) {
            removeElement(el, 1000);
        });
	}
	...
	function scoreboard(data){
		if(startup == true){
			...
			getData(data); //runs function that sets data polled from json into html objects
			startup = false; //flags that the scoreboard/getData functions have run their first pass
		} else {
			getData(data); //if startup is not set to true, only the getData function is run each time scoreboard function runs
		}
	}
	
	function getData(scObj){
		...
		var p1Bracket = scObj['p1Bracket'];
		var p2Bracket = scObj['p2Bracket'];
		...
		if(startup == true){
			...
			$('p1Bracket').html(p1Bracket);
            $('p2Bracket').html(p2Bracket);
			...
			TweenMax.to('#p1Bracket',nameTime,{css:{opacity: 1},ease:Quad.easeOut,delay:nameDelay});
			TweenMax.to('#p2Bracket',nameTime,{css:{opacity: 1},ease:Quad.easeOut,delay:nameDelay});
...
//			TweenMax.to("#p2TFlag",1,{css:{opacity: 1},delay:10,repeat:-1,repeatDelay:10,yoyo:true});
			...
			if($('#p1Bracket').text() != p1Bracket){
				TweenMax.to('#p1Bracket',.3,{css:{opacity: 0},ease:Quad.easeOut,delay:0,onComplete:function(){
					$('#p1Bracket').html(p1Bracket);
					
					TweenMax.to('#p1Bracket',.3,{css:{opacity: 1},ease:Quad.easeOut,delay:.2});
				}});
			}
			
			if($('#p2Bracket').text() != p2Bracket){
				TweenMax.to('#p2Bracket',.3,{css:{opacity: 0},ease:Quad.easeOut,delay:0,onComplete:function(){
					$('#p2Bracket').html(p2Bracket);
					
					TweenMax.to('#p2Bracket',.3,{css:{opacity: 1},ease:Quad.easeOut,delay:.2});
				}});
			}
			...
			if(enableFlags && countryHold1 != p1Country){
